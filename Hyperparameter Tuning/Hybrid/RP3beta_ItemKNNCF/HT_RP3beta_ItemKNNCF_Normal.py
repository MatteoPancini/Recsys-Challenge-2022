if __name__ == '__main__':
    ...
    partialsFile = 'RP3beta_ItemKNNCF_Normal_' + datetime.now().strftime('%b%d_%H-%M-%S')
    ...
    for index in range(len(URM_train_list)):
        ...
        recommender_ItemKNNCF_list[index].fit(ICM=ICM, shrink=189, topK=360, similarity='asym', normalization='bm25')

        recommender_RP3beta_list.append(RP3betaRecommender(URM_train_list[index]))
        recommender_RP3beta_list[index].fit(alpha=0.610567561753193, beta=0.29964622075485847, topK=101)
    ...
    recommender_ItemKNNCF.fit(ICM=ICM, shrink=189, topK=360, similarity='asym', normalization='bm25')
    ...
    with open("logs/" + "RP3beta_ItemKNNCF_Normal_Recommender" + "_logs_" + datetime.now().strftime('%b%d_%H-%M-%S') + ".json", 'w') as json_file:
