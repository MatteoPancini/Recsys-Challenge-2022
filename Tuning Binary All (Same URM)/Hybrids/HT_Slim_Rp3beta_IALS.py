if __name__ == "__main__":
    ...
    partialsFile = 'RP3beta-Slim-IALS_' + datetime.now().strftime('%b%d_%H-%M-%S')
    ...
    recommender_hybrid_Slim_RP3_list = []
    ...
    for i in range(len(URM_train_list)):
        ...
        recommender_hybrid_Slim_RP3_list.append(LinearHybridTwoRecommenderTwoVariables(URM_train_list[i], Recommender_1=recommender_RP3beta_list[i], Recommender_2=recommender_Slim_list[i]))
        recommender_hybrid_Slim_RP3_list[i].fit(alpha=0.4133522121773261, beta=0.7451419993321209)

        recommender_IALS_list.append(ImplicitALSRecommender(URM_train_list[i]))
        recommender_IALS_list[i].fit(factors=600, alpha=7, iterations=57, regularization=0.0008866558623568822)
    ...
    def objective(trial):
        ...
        for i in range(len(URM_train_list)):
            recommender_hybrid_list.append(LinearHybridTwoRecommenderTwoVariables(URM_train_list[i], Recommender_1=recommender_hybrid_Slim_RP3_list[i], Recommender_2=recommender_IALS_list[i]))
        ...
        resultsToPrint = ["RP3beta-Slim-IALS", alpha, beta, sum(MAP_result) / len(MAP_result)]
    ...
    hybrid1 = LinearHybridTwoRecommenderTwoVariables(URM_train_init, Recommender_1=rec1, Recommender_2=rec2)
    hybrid1.fit(alpha=0.4133522121773261, beta=0.7451419993321209)

    rec3 = ImplicitALSRecommender(URM_train_init)
    rec3.fit(factors=600, alpha=7, iterations=57, regularization=0.0008866558623568822)

    recommender_hybrid = LinearHybridTwoRecommenderTwoVariables(URM_train_init, Recommender_1=hybrid1, Recommender_2=rec3)
    ...
    with open("logs/" + "RP3beta-Slim-IALS" + "_logs_" + datetime.now().strftime(
