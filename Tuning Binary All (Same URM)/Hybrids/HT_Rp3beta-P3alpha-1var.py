if __name__ == "__main__":
    ...
    def objective(trial):
        ...
        for i in range(len(URM_train_list)):
            recommender_hybrid_list.append(LinearHybridTwoRecommenderOneVariable(URM_train_list[i], Recommender_1=recommender_RP3beta_list[i], Recommender_2=recommender_P3alpha_list[i]))
            recommender_hybrid_list[i].fit(alpha=alpha)

        MAP_result = evaluator_validation.evaluateRecommender(recommender_hybrid_list)

        resultsToPrint = ["RP3beta-P3alpha", alpha, sum(MAP_result) / len(MAP_result)]
