if __name__ == "__main__":
    ...
    from Recommenders.Hybrid.LinearHybridRecommender import LinearHybridTwoRecommenderOneVariable, LinearHybridTwoRecommenderTwoVariables
    ...
    def objective(trial):
        ...
        for i in range(len(URM_train_list)):
            recommender_hybrid_list.append(LinearHybridTwoRecommenderOneVariable(URM_train_list[i], Recommender_1=recommender_hybrid1_list[i], Recommender_2=recommender_Slim_list[i]))
            recommender_hybrid_list[i].fit(alpha=alpha)

        MAP_result = evaluator_validation.evaluateRecommender(recommender_hybrid_list)

        resultsToPrint = ["RP3betaP3alpha-Slim", alpha, sum(MAP_result) / len(MAP_result)]
    ...
    recommender_hybrid = LinearHybridTwoRecommenderOneVariable(URM_train_init, Recommender_1=rec1, Recommender_2=rec2)
