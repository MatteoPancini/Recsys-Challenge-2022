if __name__ == "__main__":
    ...
    partialsFile = 'IALS_SLIM_RP3beta_ItemKNNCF' + datetime.now().strftime('%b%d_%H-%M-%S')
    ...
    for index in range(len(URM_train_list)):
        ...
        recommender_hybrid2_list.append(LinearHybridTwoRecommenderTwoVariables(URM_train_list[index], Recommender_1=recommender_hybrid_RP3beta_ItemKNN_list[index], Recommender_2=recommender_SLIM_list[index]))
        recommender_hybrid2_list[index].fit(alpha=0.997101180232236, beta=0.31968335027659)

        recommender_IALS_list.append(ImplicitALSRecommender(URM_train_list[index], verbose=False))
        recommender_IALS_list[index].fit(alpha=3, factors=400, iterations=36, regularization=0.001)
    ...
    def objective(trial):
        ...
        for index in range(len(URM_train_list)):
            ...
            recommender_hybrid_list.append(LinearHybridTwoRecommenderTwoVariables(URM_train_list[index], Recommender_1=recommender_hybrid2_list[index], Recommender_2=recommender_IALS_list[index]))
        ...
        resultsToPrint = ["IALS_SLIM_RP3beta_ItemKNNCF", alpha, beta, sum(MAP_result) / len(MAP_result)]
    ...
    recommender_hybrid_triple = LinearHybridTwoRecommenderTwoVariables(URM_train_init, Recommender_1=recommender_hybrid_RP3beta_ItemKNN, Recommender_2=recommender_SLIM)
    recommender_hybrid_triple.fit(alpha=0.997101180232236, beta=0.31968335027659)

    recommender_IALS = ImplicitALSRecommender(URM_train_init, verbose=False)
    recommender_IALS.fit(alpha=3, factors=400, iterations=36, regularization=0.001)

    recommender_hybrid = LinearHybridTwoRecommenderTwoVariables(URM_train=URM_train_init, Recommender_1=recommender_hybrid_triple, Recommender_2=recommender_IALS)
