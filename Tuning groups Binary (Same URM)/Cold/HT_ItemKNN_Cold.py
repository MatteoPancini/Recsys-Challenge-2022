if __name__ == "__main__":
    ...
    partialsFile = 'ItemKNN_' + datetime.now().strftime('%b%d_%H-%M-%S')
    ...
    def objective(trial):
        ...
        for index in range(len(URM_train_list)):
            ...
            recommender_ItemKNN_list[index].fit(topK=topK, shrink=shrink)

        MAP_result = evaluator_validation.evaluateRecommender(recommender_ItemKNN_list)

        resultsToPrint = [recommender_ItemKNN_list[0].RECOMMENDER_NAME, topK, shrink,
    ...
    recommender_RP3beta = ItemKNNCFRecommender(URM_train_init, verbose=False)
    recommender_RP3beta.fit(topK=topK, shrink=shrink)
    ...
    with open("logs/" + recommender_RP3beta.RECOMMENDER_NAME + "10_logs_" + datetime.now().strftime(
