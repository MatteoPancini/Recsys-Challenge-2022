if __name__ == '__main__':
    ...
    partialsFile = 'RP3Beta-SlimEN' + datetime.now().strftime('%b%d_%H-%M-%S')
    ...
    def objective(trial):
        ...
        for index in range(len(URM_train_list)):
            recommender_SlimBPR_list.append(MultiThreadSLIM_SLIMElasticNetRecommender(URM_train_list[index]))
            recommender_SlimBPR_list[index].fit(topK=216, alpha=0.09376418450176816, l1_ratio=0.03954091993785014)

            recommender_RP3beta_list.append(RP3betaRecommender(URM_train_list[index]))
            recommender_RP3beta_list[index].fit(alpha=0.5715315472066637, beta=0.3918931238022657, topK=681)

            recommender_Hybrid_list.append(LinearHybridTwoRecommenderTwoVariables(URM_train=URM_train_list[index],
                                                                                  Recommender_1=
                                                                                  recommender_SlimBPR_list[index],
                                                                                  Recommender_2=
    ...
    recommender_SlimBPR = MultiThreadSLIM_SLIMElasticNetRecommender(URM_train_init)
    recommender_SlimBPR.fit(topK=216, alpha=0.09376418450176816, l1_ratio=0.03954091993785014)

    recommender_RP3beta = RP3betaRecommender(URM_train_init)
    recommender_RP3beta.fit(alpha=0.5715315472066637, beta=0.3918931238022657, topK=681)

    recommender_Hybrid = LinearHybridTwoRecommenderTwoVariables(URM_train=URM_train_init,
                                                                Recommender_1=recommender_SlimBPR,
    ...
    with open("logs/RP3Beta-SlimEN" + "_logs_" + datetime.now().strftime(
